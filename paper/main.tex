...
\abstract{
Современные исследования больших языковых моделей чаще всего описывают качество через число параметров, объем данных и вычислительный бюджет, оставляя геометрию функции потерь преимущественно качественным дополнением.
В работе количественно анализируется динамика ландшафтной меры как среднего разброса матриц Гессе по объектам обучения вдоль траектории оптимизации языковых нейросетевых моделей и ее связь с валидационной ошибкой.
Мера задает компактную характеристику локальной неоднородности ландшафта и дополняет показатели качества сведениями о геометрии риска, причем численное значение опирается на малую подвыборку контекстов и сравнение гессианов по спектральной норме с пересэмплированием на контрольных точках, поэтому отражает выборочную локальную информацию, а не полный набор свойств модели.
Установлено, что траектория меры в общем случае не сводится к траектории ошибки: при монотонном улучшении ошибки мера может вести себя немонотонно, в том числе возрастая на поздних участках обучения.
...
Типичный анализ опирается на срезы фиксированного бюджета и сопоставление режимов с разным соотношением числа параметров и числа токенов.
Одним из современных анализов моделей глубокого обучения является качественный анализ ландшафта функции потерь и его связи с качеством модели~\cite{choromanska2015,li2018visualizing,fort2019emergent,fort2019large}.
...
Большое число современных исследований посвящено спектральным свойствам матрицы Гессе эмпирического риска и их связи с динамикой оптимизации и организацией ландшафта~\cite{sagun2018,ghorbani2019,papyan2019,pennington2017,wu2022dissecting,xie2022powerlaw}.
...
В исследовании~\cite{wu2022dissecting} выделяются общие закономерности структуры Гессиана по слоям и типам параметров, то есть анализируется не только спектр одной агрегированной матрицы, но и устойчивые шаблоны внутри архитектуры.
В~\cite{xie2022powerlaw} для глубоких сетей описаны степенные закономерности в спектре Гессиана и их связь с масштабом модели и данными.

Отдельная линия исследований связывает обобщение и устойчивость с геометрией минимума около найденного решения~\cite{hochreiter1994,neyshabur2017exploring,dinh2017sharp}.
...
В работе~\cite{neyshabur2017exploring} систематизируются эмпирические закономерности обобщения в глубоких сетях и анализируется связь качества с режимом оптимизации~\cite{hochreiter1994} и геометрией ландшафта без сведения к одному скалярному показателю неоднородности разброса матриц Гессе между объектами обучения.
В исследовании~\cite{dinh2017sharp} показывается, что формулировки про острые минимумы как худшие для обобщения не универсальны.
В частности, примеры и теоретические конструкции демонстрируют случаи хорошего обобщения при остром минимуме, то есть постановка опирается на чувствительность решения к возмущениям в окрестности минимума, а не на разброс Гессе по обучающим точкам.

Дополняющая картина приходит из качественного анализа поверхности потерь.
...
Ближайшая к настоящей работе линия~--- конструкция ландшафтной меры на основе матриц Гессе по объектам обучения.
...
Для полносвязных архитектур в задаче классификации в~\cite{kiselev2024} построены спектральные мажоранты и верхние оценки на ландшафтную меру сложности в фиксированном оптимуме.
...
В пределе динамика становится линейной в функциональном пространстве относительно фиксированного ядра, что по смыслу отличается от режима сетей конечной ширины и от неоднородности разброса матриц Гессе по объектам, которую отражает локальная ландшафтная мера сложности.

Параллельно изучаются скалярные характеристики кривизны в точке~$\theta$. Рассматривается след матрицы Гессе, максимальное собственное значение, плотность спектра и производные от них~\cite{ghorbani2019,papyan2019,wu2022dissecting,xie2022powerlaw,petrov2025curvaturegap}.
Такие величины описывают агрегированную кривизну риска в окрестности~$\theta$ и дополняют, но не заменяют, показатели межобъектного рассогласования гессианов, вводимых в работе~\cite{kiselev2024}.
...
Эмпирические функции потерь на валидационной и обучающей выборках задаются отображениями~$\mathcal{L}_{\mathrm{val}}(\theta)$ и~$\mathcal{L}_{\mathrm{train}}(\theta)$ соответственно.
...
Здесь~$x_i$~--- токенизированный контекст фиксированной длины~$L_{\mathrm{ctx}}=128$, тогда функция ошибки для языкового моделирования имеет вид
\[
...
При~$\theta=\theta^\star$ величина~$\mu(\theta)$ совпадает с ландшафтной мерой сложности в постановке~\cite{kiselev2024}.

Величина~$\mu$ по смыслу отличается от традиционных скалярных показателей ``остроты'' по спектру агрегированного Гессиана~\cite{ghorbani2019,papyan2019,wu2022dissecting,xie2022powerlaw}: последние характеризуют одну матрицу в точке~$\theta$, тогда как~\eqref{eq:mu-def} измеряет разброс семейства~$\{\mathbf{H}_i\}$ между обучающими контекстами.
...
Далее по полному вектору обучаемых параметров производится вычисление гессиан-векторных произведений и величин~$\|(\mathbf{H}_i-\bar{\mathbf{H}})v_j\|_2$ без диагональной аппроксимации и без ручного ограничения подмножества параметров.
...
Во всех экспериментах для динамики по токенам показаны траектории выборочных средних и~$95\%$ доверительные интервалы для матожидания по распределению запусков.
...
Из этой диаграммы следует, что одному и тому же интервалу значений~$\mathcal{L}_{\mathrm{val}}$ могут соответствовать разные~$\mu$, причем разброс зависит от числа обработанных токенов и от номера независимого запуска.
Отсюда следует следующий эксперимент, в рамках которого анализируется можно ли основную закономерность описать простой совместной зависимостью~$\mu$ от ошибки на валидации и от времени обучения.
...
На общем пуле трех архитектур согласование умеренное~$R^2\approx 0{,}35$.
...
Тот же линейный каркас проверяется на данных первой части эксперимента, чтобы сопоставить две части эксперимента между собой.
...
\hline
Модель & $R^2$ & $\bar R^2$ & $F$ vs M0 & $p$ & примечание \\
\hline
M0 & $0.350$ & $0.330$ & -- & -- & базовая \\
...
$\mu$ здесь не прокси ошибки, а индикатор геометрического режима локальной неоднородности риска по обучающим срезам.
...
В принятых условиях это ослабляет гипотезу о том, что наблюдаемая немонотонность целиком объясняется шумом оценщика.
...
Концептуально результат согласуется с разделением ролей, где для опорной точки в постановке классификации в~\cite{kiselev2024} построены теоретические верхние оценки на~$\mu$, а в настоящей работе тот же показатель оценивается выборочно на пробной мини-выборке вдоль траектории обучения языковых моделей и используется для эмпирического сравнения архитектур.
...
\emph{Proceedings of the 36th International Conference on Machine Learning}.~-- 2019.~-- Vol.~97 (PMLR).~-- P.~2232--2241.
...
\emph{Advances in Neural Information Processing Systems}.~-- 2022.~-- Vol.~35. (preprint arXiv:2203.15556).
...
\textit{Statistical Learning Theory}.~-- New York: Wiley, 1998.
...
\emph{Proceedings of ICML}.~-- 2017.~-- P.~2798--2806.
...
\emph{Journal of Machine Learning Research}.~-- 2019.~-- Vol.~20(63).~-- P.~1--17.
...
\emph{Proceedings of the 2024 Ivannikov ISP RAS Open Conference (ISPRAS)}.~-- 2024.~-- P.~1--10. IEEE Xplore. DOI: \texttt{10.1109/ispras64596.2024.10899113}.
...
\emph{Proceedings of the 36th International Conference on Machine Learning}.~-- 2019.~-- Vol.~97 (PMLR).~-- P.~5012--5021.
...
\emph{Proceedings of AISTATS}.~-- 2015.~-- P.~192--204.
...
\emph{Advances in Neural Information Processing Systems}.~-- 2018.~-- Vol.~31.~-- P.~6391--6401.

\bibitem{fort2019large}
{\MakeUppercase{Fort~S., Jastrzebski~S.}}
\textit{Large scale structure of neural network loss landscapes}~//
\emph{Advances in Neural Information Processing Systems}.~-- 2019.~-- Vol.~32.~-- P.~6706--6714.
...
\emph{International Conference on Learning Representations (ICLR)}.~-- 2021. (OpenReview; preprint arXiv:2010.04261).

\bibitem{xie2022powerlaw}
{\MakeUppercase{Xie~Z., Tang~Q.-Y., Cai~Y., Sun~M., Li~P.}}
\textit{On the power-law Hessian spectrums in deep learning}~//
\emph{arXiv preprint arXiv:2201.13011}.~-- 2022. (CNNs and LLMs: Tang Q.-Y. et al., \textit{Investigating the Overlooked Hessian Structure: From CNNs to LLMs}, \emph{Proc. ICML}, PMLR 267:58805--58831, 2025).

\bibitem{hochreiter1994}
{\MakeUppercase{Hochreiter~S., Schmidhuber~J.}}
\textit{Simplifying neural nets by discovering flat minima}~//
\emph{Advances in Neural Information Processing Systems}.~-- 1995.~-- Vol.~7.~-- P.~529--536.
...
\emph{Advances in Neural Information Processing Systems}.~-- 2017.~-- Vol.~30.~-- P.~5947--5956. (preprint arXiv:1706.08947).

\bibitem{dinh2017sharp}
{\MakeUppercase{Dinh~L., Pascanu~R., Bengio~S., Bengio~Y.}}
\textit{Sharp minima can generalize for deep nets}~//
\emph{Proceedings of ICML}.~-- 2017.~-- P.~1019--1028.
...
\emph{arXiv preprint arXiv:2510.16927}.~-- 2025. (preprint metadata: submitted to ICLR 2026).
...
\emph{International Conference on Learning Representations (ICLR)}.~-- 2018. (OpenReview; preprint arXiv:1706.04454).
...
\emph{ICML 2024 Workshops: HiLD}.~-- 2024. (preprint arXiv:2406.16300).
...
\emph{International Conference on Learning Representations (ICLR)}.~-- 2022. (preprint arXiv:2203.07337).
...
\emph{Advances in Neural Information Processing Systems}.~-- 2018.~-- Vol.~31.~-- P.~8580--8589.
...
\emph{Advances in Neural Information Processing Systems}.~-- 2019.~-- Vol.~32.~-- P.~8570--8581.
